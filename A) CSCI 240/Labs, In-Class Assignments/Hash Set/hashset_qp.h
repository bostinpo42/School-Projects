...
class HashSetQP {
public:
    HashSetQP(std::function<int(T)> hash) : store{new Bucket<T>[DEFAULT_SIZE]}, currentSize{DEFAULT_SIZE}, itemQty{0}, hash{hash} { }
...
private:
    ...
    bool insertProbe(const T& item) {
        ...
        while(store[(index + (j * j)) % currentSize ].status == Occupied && count < MAX_PROBE_ATT) {
